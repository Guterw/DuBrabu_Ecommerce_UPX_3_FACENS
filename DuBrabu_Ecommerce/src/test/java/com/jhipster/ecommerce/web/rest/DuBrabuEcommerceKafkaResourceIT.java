...
import com.jhipster.ecommerce.config.EmbeddedKafka;
import com.jhipster.ecommerce.config.KafkaSseConsumer;
import com.jhipster.ecommerce.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DuBrabuEcommerceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/du-brabu-ecommerce-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/du-brabu-ecommerce-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/du-brabu-ecommerce-kafka/unregister"));
                return;
            }
        }
